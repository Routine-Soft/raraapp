  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editandoId) {
        ...
        await axios.patch(`http://192.168.247.100:8080/midiaLocal/patch/${editandoId}`, 
        ...
        // Criar nova mídia
        await axios.post('http://192.168.247.100:8080/midiaLocal/post',
          { data, hora, titulo, texto, igreja: igreja },
          { headers: { 'Content-Type': 'application/json', Authorization: token } }
        );
    ...
    Alert.alert('Confirmação', 'Deseja deletar esta mídia?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            const token = await AsyncStorage.getItem('token');
            await axios.delete(`http://192.168.247.100:8080/midiaLocal/delete/${id}`, {
              headers: { 'Content-Type': 'application/json', Authorization: token },
            });
